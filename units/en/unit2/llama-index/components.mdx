...
![RAG](https://huggingface.co/datasets/agents-course/course-images/resolve/main/en/unit2/llama-hub.png)
...
LlamaIndex makes it easy to set up storage - we can either configure a `StorageContext` manually or let LlamaIndex handle it automatically when creating a search index.
When we save the `StorageContext`, it neatly organizes all our data into files that we can easily access later.
