...
As introduced in the [section on the LlamaHub](llama-hub.mdx), we can install the ChromaDB vector store with the following command:
...
As introduced in the [section on the LlamaHub](llama-hub.mdx), we can install the LlamaTrace callback from Arize Phoenix with the following command:
