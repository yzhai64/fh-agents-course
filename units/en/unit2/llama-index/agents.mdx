...
You'll notice that agents in `LlamaIndex` are async. If you are new to async code in Python, or need a refresher, they have an [excellent async guide](https://docs.llamaindex.ai/en/stable/getting_started/async_python/).
...
query_engine_agent = AgentWorkflow.from_tools([query_engine_tool], llm=llm)
```
