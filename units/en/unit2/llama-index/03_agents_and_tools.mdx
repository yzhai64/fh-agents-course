...
An agent supports both chat and query endpoints with `query()` and `chat()` respectively.
...
### Agents with QueryEngineTools

It is easy to **wrap `QueryEngine` as tools** for an agent.
When doing so, we need to **define a name and description within the `ToolMetadata`** to improve the agent's reasoning context.
...
> There is a lot more to discover about agents and tools in LlamaIndex within the [Agent Guides](https://docs.llamaindex.ai/en/stable/module_guides/deploying/agents/modules/)
