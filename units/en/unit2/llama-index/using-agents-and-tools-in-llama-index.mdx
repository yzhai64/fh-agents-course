...
LlamaIndex  allows to define a `Tool` as well as a `ToolSpec` containing a series of customise functions under the hood.
...
Now that we understand the basics of agents and tools in LlamaIndex, let's see how we can use LlamaIndex to create configurable workflows!