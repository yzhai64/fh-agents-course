...
As we learned in the previous section, each Agent needs **an AI Model at its core**. And the currently most used AI model for those are LLMs (Large Language Model).

So the current step is to understand what are LLMs (Large Language Models) and how they are used by Agents.

This section will provide a concise technical explanation but if you’re **really not yet familiar with LLMs**, you should check first our [free Natural Language Processing Course](https://huggingface.co/learn/nlp-course/chapter1/1).
