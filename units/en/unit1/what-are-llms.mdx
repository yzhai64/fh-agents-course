...
Therefore, we need now to understand what LLMs are and how they power Agents.
...
Although Large Language Models come in various forms, LLMs are most usually decoder-based models with billions of parameters. Here are some of the most well-known LLMs:
...
Each LLM has some **special tokens** specific to the model. One important special token is the **End of sequence token** (EOS), which the model emits when it considers the sequence being completed has finished.
...
Now that we understand how LLMs work, it's time to see **how these LLM models structure their generations in a conversational context**.