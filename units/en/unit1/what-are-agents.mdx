...
Before fulfilling the order, Alfred engages in **reasoning and planning** to determine the step of actions he needs to make, and which tools to use.
...
Now that we have the big picture, let's formally define an agent as follows :
...
- The AI model performs the reasoning and generate **Actions** to execute.
...
Now that we understand what Agents are, let's take a short, ungraded quiz to reinforce our knowledge. 

After that, we'll dive deeper into the "Agent's brain": the [LLMs](what-are-llms).
