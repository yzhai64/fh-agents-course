...
- And the scope of what is possible depends on what the model has been equipped with by it's creator: a human by not having "wings" can't execute the **Action** "fly" while it is possible to execute the **Actions** "walk", "run" ,"jump", "grab" and so on.
