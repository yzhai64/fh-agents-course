...
<img src="https://huggingface.co/datasets/agents-course/course-images/resolve/main/en/unit1/uwhiteboard-no-check.jpg" alt="Unit 1 planning"/>

So, the first step, is to understand **what an Agent is**. 
...
Once he has a plan, he **must act**. To execute his plan, **he can use tools in the list of tools he knows about**. In this case, to make a coffee, he uses a coffee machine. He activates the coffee machine to brew the coffee.
...
You can think of the AI model as the **brain of the agent** and the framework as the **remaining body parts**:


- The **AI model** performs the reasoning and generates **Actions** to execute.  

- The **scope of possible actions** depends on what the model **has been equipped with**: it's like a human by not having "wings" can't execute the "fly" **Action**, while it is possible to execute **Actions** "walk", "run" ,"jump", "grab", and so on.
...
However, if you have used chat applications like HuggingChat, ChatGPT or Le Chat, you will have noticed that they are able to generate images too! How is that possible?
...
Now that we have a solid grasp of what Agents are, let’s reinforce your understanding with a short, ungraded quiz. After that, we’ll dive into the “Agent’s brain”: the [LLMs](what-are-llms).