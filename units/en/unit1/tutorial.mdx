...
I provided my agent with an **Image generation tool** and asked it to generate an image of a cat.
...
> Thanks to [Aymeric](https://huggingface.co/spaces/m-ric/agent) for this template! 
...
here are some simple but necessary imports
```python
...
We will give more in depth example when we will present the framework in a later unit, but the import here is to add the list of tools to the the **tools** parameter of your agent.
