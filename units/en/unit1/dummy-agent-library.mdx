...
You can find the notebook [here](https://colab.research.google.com/drive/1MmVo8V9bLkgcVA1RB5LZtX_lO8y_HBxm?usp=sharing). If you **want to run the code**.
...
If we now add the special tokens related to the <a href="https://huggingface.co/meta-llama/Llama-3.2-3B-Instruct">Llama-3.2-3B-Instruct model</a> that we're using, the behavior changes and now produces the expected EOS.
...
Now, we're ready **to create our first real Agent** using `smolagents` library.
