...
You can find the notebook [here](INSER LINK to the notebook section).

## Serverless API
In the Hugging Face ecosystem there is a conveniant feature called Serverless API that allows you to test some models. This uses a shared enpoints between multiples users to compute some requests. This is conveniant to test some models.
...
If we now add the special tokens related to the Llama-3.2-3B-Instruct model that we're using, the behaviour changes and is now the expected oen.
```python
...
Paris.
```
The chat method is the RECOMMANDED method to use in order to ensure a smooth transition between models but since this notebook is only educationnal, we will keep using the "text_generation" method to understand the details.
