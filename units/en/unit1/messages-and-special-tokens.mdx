...
Now that we understood how LLMs work, it’s time to see how they actually **structure their generations through chat templates**.

Since you interact with Agents through a chat interface, it’s important to study the underlying Message System that turns user inputs and system instructions into a coherent prompt.
...
System messages (also known as System Prompts) set the foundation for **how the model should behave**. They act as **persistent instructions that influence all subsequent interactions**. 
...
Based on our System Message, Alfred is polite and helpful.
...
But if we change System Message for:
...
In addition to setting behavior, the System Message in Agents is also used  **to store information about available tools**, provide instructions to the model on how to format the actions to take, and guide the overall guidelines of how the thought process should be segmented.
...
Chat templates help maintain context by preserving conversation history, storing previous exchanges between the user and the assistant. This leads to more coherent multi-turn conversations. For example:
...
As we explained in the introduction, chat templates are essential for structuring interactions between language models and users. They provide instruction to format message conversations into prompt.
...
__
Now that we learned how LLMs  We can focus on the Agent, and understand how we define tools in the System Message.

Now that we’ve explored how LLMs structure their generations through chat templates and why, it’s time to see how Agents take action in their environments. One of the key ways they do this is by using Tools, functions that extend an AI model’s capabilities beyond just text generation. 

We will talk again in the upcoming units on Message, but if you want to dive deeper you can check these two documentations:
