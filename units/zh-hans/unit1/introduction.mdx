# 代理简介 (Introduction to Agents)
<img src="https://huggingface.co/datasets/agents-course/course-images/resolve/main/en/unit1/thumbnail.jpg" alt="Thumbnail"/>

欢迎来到第一单元，在这里**你将在AI代理 (AI Agents) 的基础知识中建立坚实的基础**，包括：

* **理解代理 (Understanding Agents)**
   * 什么是代理，它是如何工作的？
   * 代理如何使用推理 (reasoning) 和规划 (planning) 做出决策？

* **大型语言模型 (LLMs) 在代理中的角色**
   * LLMs如何作为代理的"大脑"
   * LLMs如何通过消息系统 (Messages system) 构建对话
...
探索这些主题后，**你将使用`smolagents`构建你的第一个代理**！

你的代理名为Alfred，将处理一个简单的任务，并展示如何在实践中应用这些概念。

你甚至会学习如何**在Hugging Face Spaces上发布你的代理**，这样你就可以与朋友和同事分享它。

最后，在本单元结束时，你将参加一个测验。通过它，你将**获得你的第一个课程认证**：🎓 代理基础证书 (Certificate of Fundamentals of Agents)。
...
这个单元是你的**重要起点**，在进入更高级的主题之前，为理解代理打下基础。
