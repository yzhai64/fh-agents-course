...
它的工作原理是**将较少数量的新权重作为适配器插入到模型中进行训练**。这使得使用 LoRA 进行训练更快、内存效率更高，并产生更小的模型权重（几百MB），更易于存储和共享。
...
你可以在这里访问教程笔记本 👉 [这里](https://huggingface.co/agents-course/notebooks/blob/main/bonus-unit1/bonus-unit1.ipynb)。

然后，点击 Open In Colab 以便在 Colab Notebook 中运行它。