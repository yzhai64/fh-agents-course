...
Agent trong `smolagents` sẽ có **hành vi giống với agent tự build trước đây**: nó sẽ **suy nghĩ, hành động và quan sát theo chu kỳ** cho đến khi có câu trả lời cuối:
...
Agent sử dụng [`Qwen/Qwen2.5-Coder-32B-Instruct`](https://huggingface.co/Qwen/Qwen2.5-Coder-32B-Instruct) làm LLM engine. Đây là model mạnh mẽ mà ta sẽ truy cập qua serverless API.
