...
若需了解 smolagents 和 Hugging Face 支持的其他替代方案，请参考<a href="./why_use_smolagents.mdx">为什么选择smolagents</a>章节。
</Tip>
...
我们需要一组专为浏览设计的代理工具，例如“search_item_ctrl_f”、“go_back”和“close_popups”。这些工具允许代理像浏览网页的人一样行事。
...
我们还需要保存屏幕截图的功能，因为这是我们的 VLM 代理完成任务时必不可少的一部分。此功能会捕获屏幕截图并将其保存在 `step_log.observations_images = [image.copy()]` 中，从而允许代理在导航时动态存储和处理图像。
...
此函数作为 `step_callback` 传递给代理，因为它在代理执行的每一步结束时被触发。这使得代理能够在整个过程中动态捕获和存储屏幕截图。

现在，我们可以生成用于浏览网页的视觉代理，为其提供我们创建的工具，以及 `DuckDuckGoSearchTool` 以探索网页。此工具将帮助代理根据视觉线索检索验证访客身份所需的信息。
...
```python
agent.run("""
我是韦恩庄园的管家 Alfred，负责核实参加派对的宾客身份。一位超级英雄来到入口处，声称自己是神奇女侠，但我需要确认她是否就是她说的那个人。

请搜索神奇女侠的图像，并根据这些图像生成详细的视觉描述。此外，导航到维基百科以获取关于她外貌的关键信息。利用这些信息，我可以决定是否允许她进入活动。
""" + helium_instructions)
```

您可以看到，我们将 `helium_instructions` 作为任务的一部分包含在内。这个特殊的提示旨在控制代理的导航，确保它在浏览网页时遵循正确的步骤。
...
```python
最终答案：神奇女侠通常被描绘成穿着红色和金色的紧身衣，蓝色短裤或短裙上有白色星星，戴着金色王冠，戴着银色手镯，手持金色真言套索。她是提坦尼亚岛的公主戴安娜，也被称作人类世界中的戴安娜·普林斯。
```
...
- [我们让 smolagents 有了视觉能力](https://huggingface.co/blog/smolagents-can-see) -  博客文章描述了视觉代理的功能。
- [使用智能体进行网页浏览 🤖🌐](https://huggingface.co/docs/smolagents/examples/web_browser) - 使用视觉代理进行网页浏览的示例。
- [网页浏览视觉代理示例](https://github.com/huggingface/smolagents/blob/main/src/smolagents/vision_web_browser.py) - 使用视觉代理进行网页浏览的示例。