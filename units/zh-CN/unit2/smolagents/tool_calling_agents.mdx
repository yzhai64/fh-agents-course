...
工具调用代理（Tool Calling Agents）是 `smolagents` 中提供的第二种代理类型。与使用 Python 代码片段的代码代理（Code Agents）不同，这类代理**利用 LLM 提供商的内置工具调用能力**来生成 **JSON 结构**的工具调用指令。这是 OpenAI、Anthropic 等主流提供商采用的标准方法。
...
工具调用代理遵循与代码代理相同的多步骤工作流程（详见[前一章节](./code_agents)）。关键区别在于**操作结构方式**：代理不再生成可执行代码，而是**生成指定工具名称和参数的 JSON 对象**，系统随后**解析这些指令**来执行相应工具。

## 示例：运行工具调用代理

让我们重新审视 Alfred 开始筹备派对的示例，这次使用 `ToolCallingAgent` 来展示区别。我们将构建一个能够使用 DuckDuckGo 进行网页搜索的代理，与代码代理示例的唯一区别在于代理类型——框架会处理其他所有细节：
...
当查看代理的执行跟踪时，您将看到类似以下内容而非 `Executing parsed code:`：
...
代理生成结构化的工具调用指令，系统通过处理这些指令来生成输出，而非像 `CodeAgent` 那样直接执行代码。

现在我们已经了解两种代理类型，可以根据需求选择合适的一种。让我们继续探索 `smolagents`，让 Alfred 的派对大获成功！🎉
