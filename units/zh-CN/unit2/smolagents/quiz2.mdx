...
现在该测试您对*代码代理*、*工具调用代理*和*工具*章节的理解了。本测验为可选且不计分。
...
choices={[
  ...
  {
    text: "<code>@tool</code> 只能用于多代理系统，而创建 <code>Tool</code> 的子类适用于单代理场景",
    explain: "所有代理（单或多）都可以使用这两种方法定义工具，没有此类限制。",
  },
  {
...
choices={[
  {
    text: "它将每个步骤传递给多代理系统中的不同代理，然后合并结果",
    explain: "尽管多代理系统可以分配任务，但 CodeAgent 本身可以使用 ReAct 独立处理多个步骤。",
  },
  {
...
choices={[
  ...
  },
  {
    ...
    explain: "CodeAgent 和 ToolCallingAgent 都可以调用共享工具，没有基于代理类型的限制。",
  },
  {
...
choices={[
  {
    text: "ToolCallingAgent 仅兼容多代理系统，而 CodeAgent 可以单独运行",
    explain: "两种代理都可以单独使用或作为多代理系统的一部分。",
  },
  {
    text: "ToolCallingAgent 将所有推理委托给单独的检索代理，然后返回最终答案",
    explain: "ToolCallingAgent 仍然使用主 LLM 进行推理，不完全依赖检索代理。",
  },
  {
...
choices={[
  {
    ...
    explain: "正确。默认工具箱包含这些现成工具，便于在构建代理时快速集成。",
    correct: true
  },
  {
  ...
  {
    text: "它专门为多代理系统设计，与单 CodeAgent 不兼容",
    explain: "默认工具箱适用于任何代理类型，包括单代理和多代理设置。",
  },
  {
...
恭喜完成测验！🎉 如果有任何问题让您感到困难，请重新访问*代码代理*、*工具调用代理*或*工具*章节以加强理解。如果您表现出色，那么您已踏上构建强大 smolagents 应用的道路！
