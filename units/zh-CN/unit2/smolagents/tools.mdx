...
正如我们在[第一单元](https://huggingface.co/learn/agents-course/unit1/tools)所探讨的，代理通过工具执行各类操作。在`smolagents`框架中，工具被视为 **LLM 可以在代理系统中调用的函数**。
...
![来自 https://huggingface.co/docs/smolagents/conceptual_guides/react 的代理流程](https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/transformers/Agent_ManimCE.gif)
...
class SuperheroPartyThemeTool(Tool):
    ...
    description = """
    该工具根据给定类别生成超级英雄主题的派对创意
    返回独特的派对主题方案"""
    
    inputs = {
        "category": {
            "type": "string",
            "description": "派对主题分类（如：'经典英雄'、'反派假面舞会'、'未来哥谭'）",
        }
    }
    ...
    def forward(self, category: str):
        themes = {
            "classic heroes": "正义联盟晚宴：宾客装扮成DC经典英雄，提供'氪石特饮'等主题鸡尾酒",
            "villain masquerade": "哥谭恶棍舞会：神秘假面派对，宾客装扮成经典蝙蝠侠反派角色",
            "futuristic Gotham": "新哥谭之夜：受《蝙蝠侠：未来》启发的赛博朋克风格派对，配备霓虹装饰与未来科技装置"
        }
        
        return themes.get(category.lower(), "未找到匹配主题。请尝试'经典英雄'、'反派假面舞会'或'未来哥谭'")
...
# 运行代理生成派对主题
result = agent.run(
    "为'反派假面舞会'主题推荐一个超级英雄派对的创意方案"
)

print(result)  # 输出："哥谭恶棍舞会：神秘假面派对，宾客装扮成经典蝙蝠侠反派角色"
```
...
`smolagents` 自带一组预构建工具，可直接注入到您的代理中。[默认工具箱](https://huggingface.co/docs/smolagents/guided_tour?build-a-tool=Decorate+a+function+with+%40tool#default-toolbox) 包含:  
...
agent.run("生成韦恩庄园豪华超级英雄主题派对的图像，包含虚构的超级英雄。")
```
...
- [构建高效代理](https://huggingface.co/docs/smolagents/tutorials/building_good_agents) - 关于开发可靠高性能自定义函数代理的最佳实践指南